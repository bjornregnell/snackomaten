...
case class Message(text: String):
  ...
  private def get(key: String): Option[String] =
    ...
    if startPos < 0 then None else 
      val untilPos = if key == Key.Body.keyString then text.length else
  ...
  lazy val time:   Option[Timestamp] = get(Key.Time).flatMap(Timestamp.parse)
...
object Message:
  ...
  def apply(userId: String, cmd: Cmd, body: String): Message = 
    Message(Key.UserId(userId) + ";" 
      + Key.Command(Cmd.Send.toString) + ";" 
      + Key.Time(Timestamp.now().show) + ";" 
      + Key.Body(body))